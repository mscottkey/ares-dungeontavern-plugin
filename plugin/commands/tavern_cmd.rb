module AresMUSH
  module DungeonTavern
    ...
    class CarouseCmd
      ...
      def handle
        night = DungeonTavern.active_tavern_night(enactor.room)
        unless night
          client.emit_failure "No tavern night is active here. Someone needs to open one first."
          return
        end
        participant = DungeonTavern.find_or_create_tavern_participant(night, enactor)

        room_name = enactor.room ? enactor.room.name : ""
        stat_val = DungeonTavern.stat_value(enactor, "heart",
                                          move_name: "Carouse",
                                          room_name: room_name)
    ...
    class ImbibeCmd
      ...
      def handle
        ...
        limit = DungeonTavern.ineb_limit(enactor)
        ...
        if new_ineb >= limit
          enactor.room.emit t('dungeontavern.inebriation_limit', name: enactor.name)
