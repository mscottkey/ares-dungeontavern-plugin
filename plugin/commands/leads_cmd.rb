  ...
  module DungeonTavern
    class LeadsCmd
      ...
      def do_flush
        ...
        client.emit t('dungeontavern.leads_flush_header', days: days)
        ...
        open_leads.each do |lead|
          ...
          result = DungeonTavern.close_lead(lead, enactor, reason: :idle)

          case result[:outcome]
          when :converted
            output << t('dungeontavern.leads_flush_converted',
                        title: lead.title,
                        clues: lead.clues_gathered,
                        needed: lead.clues_needed)
          when :archived
            output << t('dungeontavern.leads_flush_archived',
                        title: lead.title,
                        clues: lead.clues_gathered,
                        needed: lead.clues_needed)
          when :removed
            output << t('dungeontavern.leads_flush_removed',
                        title: lead.title,
                        needed: lead.clues_needed)
          end
        ...
        if flushed == 0
          client.emit t('dungeontavern.leads_flush_none', days: days)
          return
        end
        ...
        client.emit t('dungeontavern.leads_flush_summary',
                      count: flushed,
                      active: active_count,
                      cap: global_cap)
      end
